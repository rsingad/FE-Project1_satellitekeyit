...
import { Package, ArrowRight, ShieldCheck, Fingerprint, Lock, Mail } from 'lucide-react';
...
const Login = () => {
  ...
  useEffect(() => {
    let ctx = gsap.context(() => {
      ...
      const moveHero = (e) => {
        // Only apply mouse parallax if we aren't focused on an input
  ...
  const speak = (text) => {
    return new Promise((resolve) => {
      if ('speechSynthesis' in window) {
        ...
        // Resolve the promise when the voice finishes speaking
        ...
        // Fallback: If voice doesn't trigger within 3 seconds, resolve to unblock
  ...
  const handleSubmit = async (e) => {
    ...
    // Voice prompt for verification (we don't await this one, let it play while API calls)
    speak("Verifying security credentials. Please wait.");
    ...
    if (success) {
      ...
      await speak(`Access granted. Welcome to the system, ${name}.`);
      
      navigate('/');
    } else {
      // Voice prompt for failure
  ...
  // Calculate rotation based on email length to make the 3D core "look" at the typing
  const lookRotation = Math.min(Math.max((email.length * 2) - 20, -30), 30);

  return (
    <div ref={backgroundRef} className="min-h-screen flex bg-[#030014] overflow-hidden relative selection:bg-indigo-500/30 text-slate-200">

      {/* Background Animated Orbs */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
        <div className="blob1 absolute top-[-10%] left-[-10%] w-[40vw] h-[40vw] rounded-full bg-indigo-600/20 blur-[120px] mix-blend-screen" />
        <div className="blob2 absolute bottom-[-10%] right-[-10%] w-[50vw] h-[50vw] rounded-full bg-purple-600/20 blur-[130px] mix-blend-screen" />
        <div className="blob3 absolute top-[30%] left-[40%] w-[30vw] h-[30vw] rounded-full bg-cyan-600/10 blur-[100px] mix-blend-screen" />
      ...
      <div className="flex-1 flex flex-col justify-center items-center py-12 px-4 sm:px-6 z-10 relative">
        ...
        >
          <div className="backdrop-blur-xl bg-white/[0.03] border border-white/[0.08] p-10 rounded-3xl shadow-2xl relative overflow-hidden">
            ...
            <motion.div variants={itemVariants} className="flex items-center justify-center gap-3 mb-8">
              <div className="bg-gradient-to-br from-indigo-500 to-purple-600 p-2.5 rounded-2xl shadow-lg shadow-indigo-500/30 border border-white/10">
            ...
            <motion.div variants={itemVariants} className="text-center mb-10">
              ...
              <p className="text-sm text-slate-400">Authenticate to access your secure portal.</p>
            </motion.div>
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              <motion.div variants={itemVariants}>
                ...
                <div className="relative group">
                  ...
                  <input
                    ...
                    className="block w-full pl-11 pr-4 py-3.5 bg-black/20 border border-white/10 rounded-2xl text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 focus:bg-black/40 transition-all text-sm"
              ...
              <motion.div variants={itemVariants}>
                ...
                <div className="relative group">
                  ...
                  <input
                    ...
                    onBlur={() => setFocusedField(null)}
                    className="block w-full pl-11 pr-4 py-3.5 bg-black/20 border border-white/10 rounded-2xl text-white placeholder-slate-500 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 focus:bg-black/40 transition-all text-sm"
                    placeholder="••••••••"
                    value={password}
              ...
              <motion.div variants={itemVariants} className="pt-2">
                <motion.button
                  whileHover={{ scale: 1.01, boxShadow: "0px 0px 20px rgba(99, 102, 241, 0.4)" }}
                  whileTap={{ scale: 0.97 }}
                  type="submit"
                  disabled={isLoading}
                  className="w-full flex justify-center items-center gap-2 py-4 px-4 rounded-2xl text-sm font-bold text-white bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 border border-indigo-400/30 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-[#030014] focus:ring-indigo-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed relative overflow-hidden group"
                >
      ...
      <div className="hidden lg:flex relative w-0 flex-1 items-center justify-center z-10 overflow-hidden" style={{ perspective: '1000px' }}>
        ...
        <motion.div
          animate={{
            backgroundColor: focusedField === 'password' ? 'rgba(168, 85, 247, 0.4)' : focusedField === 'email' ? 'rgba(6, 182, 212, 0.4)' : 'rgba(99, 102, 241, 0.3)',
        ...
        <motion.div
          ...
          animate={{
            rotateX: focusedField === 'email' ? 10 : focusedField === 'password' ? -20 : 0,
            rotateY: focusedField === 'email' ? lookRotation : focusedField === 'password' ? 0 : 0,
        ...
        >
          ...
          <motion.div
            animate={{
              ...
              borderColor: focusedField === 'password' ? 'rgba(168, 85, 247, 0.8)' : 'rgba(99, 102, 241, 0.3)'
            }}
            transition={{
          ...
          <motion.div
            animate={{
              ...
              borderColor: focusedField === 'password' ? 'rgba(168, 85, 247, 0.6)' : 'rgba(6, 182, 212, 0.4)'
            }}
            transition={{
          ...
          <motion.div
            ...
            animate={{
              boxShadow: focusedField === 'password' ? "0 0 80px rgba(168, 85, 247, 0.6)" : focusedField === 'email' ? "0 0 80px rgba(6, 182, 212, 0.6)" : "0 0 40px rgba(99, 102, 241, 0.4)"
            }}
          >
            <AnimatePresence mode="wait">
              {focusedField === 'password' ? (
                ...
                <motion.div
                  ...
                  transition={{ type: "spring", stiffness: 200, damping: 20 }}
                >
                  <Fingerprint size={56} strokeWidth={1} className="text-indigo-400" />
                </motion.div>
              )}
            </AnimatePresence>
          </motion.div>
        ...
        >
          <div className="flex items-center gap-2 text-xs font-bold tracking-widest uppercase text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-400">
