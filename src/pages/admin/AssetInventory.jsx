...
import { Sparkles, Upload, Plus, Edit, Trash2, AlertTriangle, X, Check, Copy, PackageSearch, Search, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function AssetInventory() {
  ...
  fixedAssets = applySearch(fixedAssets);
  consumables = applySearch(consumables);
  historyAssets = applySearch(historyAssets);

  // Pagination logic
  let activeList = activeTab === 'Fixed Assets' ? fixedAssets : activeTab === 'Consumables' ? consumables : historyAssets;
  ...
  return (
    <div ref={backgroundRef} className="min-h-screen bg-[#030014] text-slate-200 p-4 sm:p-6 lg:p-8 selection:bg-cyan-500/30 overflow-hidden relative">
      ...
      <div className="max-w-7xl mx-auto space-y-6 relative z-10">
        ...
        {/* Tabs & Search Layout */}
        <div className="flex flex-col lg:flex-row lg:items-center justify-between border-b border-white/10 mb-6 gap-4 pb-4 lg:pb-0">
          <div className="flex space-x-8 overflow-x-auto custom-scrollbar">
            {TABS.map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`pb-4 text-sm font-bold tracking-wider uppercase whitespace-nowrap relative transition-colors ${activeTab === tab ? 'text-cyan-400' : 'text-slate-500 hover:text-slate-300'
                  }`}
              >
                {tab}
                {activeTab === tab && (
          ...
          <div className="flex items-center gap-4 w-full lg:w-auto lg:pb-4">
            ...
            {(filterParam || conditionParam) && (
              <Link to="/admin/inventory" className="text-xs font-bold text-rose-400 bg-rose-500/10 border border-rose-500/30 px-3 py-2 rounded-xl hover:bg-rose-500/20 transition-colors flex items-center gap-1 whitespace-nowrap uppercase tracking-widest">
                    ...
                    <tr key={idx}>
                      {[1, 2, 3, 4, 5].map(i => (
                        <td key={i} className="px-6 py-4">
                          <div className={`h-4 bg-slate-700/30 rounded animate-pulse ${i === 1 ? 'w-32' : i === 2 ? 'w-24' : i === 5 ? 'w-16 ml-auto' : 'w-20'}`}></div>
                        </td>
                      ))}
                    </tr>
                  ))}
              ...
              {activeTab === 'Fixed Assets' && (
                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.2 }}>
                  <table className="w-full text-left text-sm border-collapse">
                    ...
                    <tbody className="divide-y divide-white/5">
                      {paginatedList.length === 0 && <tr><td colSpan="5" className="p-8 text-center text-slate-500 font-mono">No fixed assets found.</td></tr>}
                      {paginatedList.map((asset) => (
                        <tr key={asset._id} className="hover:bg-white/[0.02] transition-colors group">
                          ...
                          <td className="px-6 py-4">
                            <span className={`px-2 py-1 rounded border text-[10px] uppercase font-bold tracking-widest ${asset.status === 'Allocated' ? 'bg-blue-500/10 text-blue-400 border-blue-500/30' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30'
                              }`}>
                              {asset.status}
                            </span>
                          </td>
              ...
              {activeTab === 'Consumables' && (
                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.2 }}>
                  <table className="w-full text-left text-sm border-collapse">
                    ...
                    <tbody className="divide-y divide-white/5">
                      {paginatedList.length === 0 && <tr><td colSpan="4" className="p-8 text-center text-slate-500 font-mono">No consumables found.</td></tr>}
              ...
              {activeTab === 'Repair & Disposed' && (
                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.2 }}>
                  <table className="w-full text-left text-sm border-collapse">
                    ...
                    <tbody className="divide-y divide-white/5">
                      {paginatedList.length === 0 && <tr><td colSpan="4" className="p-8 text-center text-slate-500 font-mono">No history logs found.</td></tr>}
                      {paginatedList.map((asset) => (
                        <tr key={asset._id} className="hover:bg-white/[0.02] transition-colors text-slate-400 group">
                          ...
                          <td className="px-6 py-4">
                            <span className={`px-2 py-1 rounded border text-[10px] uppercase font-bold tracking-widest ${asset.condition === 'Scrapped' ? 'bg-slate-500/20 text-slate-400 border-slate-500/30' : 'bg-amber-500/10 text-amber-400 border-amber-500/30'
                              }`}>
                              {asset.condition}
                            </span>
                          </td>
